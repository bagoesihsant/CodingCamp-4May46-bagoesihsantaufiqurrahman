...
App.Theme = {
  ...
  init: function () {
    var saved = App.Storage.get('pd_theme', null);
  ...
  toggle: function () {
    ...
    this._apply(this._current);
    App.Storage.set('pd_theme', this._current);
  },
...
App.Greeting = {
  ...
  _renderGreeting: function () {
    ...
    var name = App.Storage.get('pd_userName', null);
    var text = phrase;
    if (name && typeof name === 'string' && name.trim().length > 0) {
      text += ', ' + name.trim().slice(0, 50);
    }
...
/* ============================================================
   App.NameInput — custom name form
   ============================================================ */
App.NameInput = {
  init: function () {
    var input = document.getElementById('name-input');
    var savedName = App.Storage.get('pd_userName', null);
    if (input && savedName) {
  ...
  _onSubmit: function (e) {
    ...
    if (trimmed.length === 0) {
      // Silently retain previous value — do not overwrite
      return;
    }
    App.Storage.set('pd_userName', trimmed);
...
App.Timer = {
  ...
  init: function () {
    var savedMinutes = App.Storage.get('pd_pomoDuration', 25);
...
App.TimerConfig = {
  ...
  _onApply: function (e) {
    ...
    var completeMsg = document.getElementById('timer-complete-msg');
    if (completeMsg) completeMsg.classList.add('hidden');
    App.Storage.set('pd_pomoDuration', result.minutes);
  }
};
...
App.Todo = {
  ...
  init: function () {
    var loaded = App.Storage.get('pd_tasks', null);
  ...
  _persist: function () {
    var result = App.Storage.set('pd_tasks', this._tasks);
    if (!result.ok) {
...
App.Links = {
  ...
  init: function () {
    var loaded = App.Storage.get('pd_links', null);
    if (Array.isArray(loaded)) {
      this._links = loaded;
    } else {
      this._links = [];
  ...
  _persist: function () {
    var result = App.Storage.set('pd_links', this._links);
    if (!result.ok) {
